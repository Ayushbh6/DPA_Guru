...
class Document(Base):
    ...
    deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    lifecycle_status: Mapped[str] = mapped_column(String(32), nullable=False, server_default=text("'active'"))
...
class ChecklistDraftJob(Base):
    ...
    stale_document_ids: Mapped[list[str]] = mapped_column(JSONB(astext_type=Text()), nullable=False, server_default=text("'[]'::jsonb"))
    meta_json: Mapped[dict | None] = mapped_column(JSONB(astext_type=Text()), nullable=True)
...
class CopilotMessage(Base):
    ...
    thread_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("copilot_threads.id", ondelete="CASCADE"), nullable=False
    )
    role: Mapped[str] = mapped_column(String(32), nullable=False)
